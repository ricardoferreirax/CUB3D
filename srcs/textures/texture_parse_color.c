...
/*   Updated: 2026/05/05 21:18:29 by rmedeiro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_floor_ceiling_color(t_game *g, const char *s, int *dest)
{
	...
	if (*dest != -1)
		exit_game(EXIT_MAP, g, "parse_floor_ceiling_color() given invalid dst");
	i = 0;
	while (s[i] == ' ' || s[i] == '\t')
		i++;
	red = read_rgb(s, &i, g);
	if (s[i] != ',')
		exit_game(EXIT_MAP, g, "parse_floor_ceiling_color() invalid r color");
	i++;
	green = read_rgb(s, &i, g);
	if (s[i] != ',')
		exit_game(EXIT_MAP, g, "parse_floor_ceiling_color() invalid g color");
	i++;
	blue = read_rgb(s, &i, g);
	while (s[i] == ' ' || s[i] == '\t')
		i++;
	if (s[i] && s[i] != '\n')
		exit_game(EXIT_MAP, g, "parse_floor_ceiling_color() invalid b color");
...
void	parse_floor_ceiling_line(t_game *g, char id, char *value)
{
	...
	strip_newline(value);
	if (!*value)
		exit_game(EXIT_MAP, g, "parse_floor_ceiling_line() invalid value");
	if (id == 'F')
	{
